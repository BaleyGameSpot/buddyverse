...
class Dashboard {
    ...
    /**
     * Read memory information from /proc/meminfo (Linux only)
     * @return array|null
     */
    private function readMemInfo()
    {
        ...
        foreach ($lines as $line) {
            if (preg_match('/^(\w+):\s+(\d+)/', $line, $matches)) {
                $data[$matches[1]] = $matches[2]; // Value in KB
            }
        }
    ...
    public function server_settings()
    {
	    ...
	    $server_settings['PHP ini files'] = (!empty(checkIniFiles()) && scount(checkIniFiles()) > 0) ? 0 : 1;
    ...
    {
    	global $tconfig, $obj;
    	$socket_cluster_status_html = isset($socket_cluster_status_html) ? $socket_cluster_status_html : '';
    ...
    public function getSystemDiagnosticData() {
    	...
    	$dataArr[] = array('title' => 'MySQL TimeZone Data Configurations', 'value' => $this->server_timezone_table(), 'modal_id' => 'server_timezone_table_modal');
	...
	public function getServerInfo() {
		...
		if ($operating_system === 'Windows') {
			...
			// Try to get connection counts using shell_exec if available
			$connectionsResult = $this->safeShellExec('netstat -nt | findstr :80 | findstr ESTABLISHED | find /C /V ""');
			if ($connectionsResult !== null) {
				$connections = (int)trim($connectionsResult);
			}

			$totalConnectionsResult = $this->safeShellExec('netstat -nt | findstr :80 | find /C /V ""');
			if ($totalConnectionsResult !== null) {
				$totalconnections = (int)trim($totalConnectionsResult);
			}
		} else {
			/*  Linux CPU */
			$load = sys_getloadavg();
			...
			if ($meminfo !== null) {
				// Read directly from /proc/meminfo (values in KB)
				$memtotal = isset($meminfo['MemTotal']) ? round($meminfo['MemTotal'] / 1000000, 2) : 0;
				$memavailable = isset($meminfo['MemAvailable']) ? round($meminfo['MemAvailable'] / 1000000, 2) : 0;
				$memfree = isset($meminfo['MemFree']) ? round($meminfo['MemFree'] / 1000000, 2) : 0;
				$memused = round($memtotal - $memavailable, 2);
				$memshared = isset($meminfo['Shmem']) ? round($meminfo['Shmem'] / 1000000, 2) : 0;
				$memcached = isset($meminfo['Cached']) ? round($meminfo['Cached'] / 1000000, 2) : 0;
			} else {
				// Fallback: Try using shell_exec with free command
				$free = $this->safeShellExec('free');
				...
				if ($free !== null) {
					...
					$memshared = isset($mem[4]) ? round($mem[4] / 1000000,2) : 0;
					$memcached = isset($mem[5]) ? round($mem[5] / 1000000,2) : 0;
					$memavailable = isset($mem[6]) ? round($mem[6] / 1000000,2) : $memfree;
				} else {
			...
			// Try to get connection counts using shell_exec if available
			$connectionsResult = $this->safeShellExec("netstat -ntu | grep :80 | grep ESTABLISHED | grep -v LISTEN | awk '{print \$5}' | cut -d: -f1 | sort | uniq -c | sort -rn | grep -v 127.0.0.1 | wc -l");
			if ($connectionsResult !== null) {
				$connections = (int)trim($connectionsResult);
			}

			$totalConnectionsResult = $this->safeShellExec("netstat -ntu | grep :80 | grep -v LISTEN | awk '{print \$5}' | cut -d: -f1 | sort | uniq -c | sort -rn | grep -v 127.0.0.1 | wc -l");
			if ($totalConnectionsResult !== null) {
				$totalconnections = (int)trim($totalConnectionsResult);
			}
		}
	...
	public function apiServiceConfig() {
    	...
    	if(!is_resource(checkSocketCluster())) {
    		return false;
    	}
    	if(!is_resource(checkMapAPIreplacementAvailable()) || !checkMapAPIService()) {
    		return false;
    	}
    	if(!$this->checkMemcacheService()) {
    		return false;
    	}
	...
	public function checkSysComponentScript() {
		global $tconfig;
		$server_ip = $this->getServerIP();
        $ALLOWED_DOMAINS_ARR = array('v3cprojectsdemo.com');
        if( (strpos_arr_h($_SERVER["HTTP_HOST"], $ALLOWED_DOMAINS_ARR) !== false) ||
            ( !empty($server_ip) && file_exists($tconfig['tsite_script_file_path'] . md5($server_ip) . '.txt') ) ) {
            return true;
        }
	...
	public function server_timezone_table() {
		global $obj;
		$server_timezone_table_info = $obj->MySQLSelect("SELECT CONVERT_TZ(NOW(), 'UTC', 'Asia/Kolkata') as timeconverted");
		if(!empty($server_timezone_table_info[0]['timeconverted']))	{
			return true;
		} else {
			return false;
		}
