...
if ($action == 'photo') {
     ...
     if ($image_name != "") {
          ...
          $imageUploadingExtenstionMsg = str_replace("####",$tconfig["tsite_upload_image_file_extensions_validation"],$langage_lbl['LBL_FILE_UPLOADING_EXTENSION_MSG']);
          $error = $validobj->validateFileType($_FILES['photo'], $tconfig["tsite_upload_image_file_extensions"], $imageUploadingExtenstionMsg);
         ...
         /* if ($_FILES['photo']['size'] > 1048576) {
          ...
          if ($error) {
              ...
              $_SESSION['session_msg'] = $error;
               getPostForm($_POST, $error, "profile_rider.php?success=0&var_msg=" . $error);
               exit;
          } else {
               ...
               $img1 = $UPLOAD_OBJ->GeneralUploadImage($image_object, $image_name, $Photo_Gallery_folder, '','','', '', '', '', 'Y', '', $Photo_Gallery_folder);
				if($img1!=''){
                    ...
                    $q = "UPDATE ";
                    $query = $q . " `" . $tbl . "` SET `vImgName` = '" . $vImgName . "' " . $where;
                    $obj->sql_query($query);
                    getProfileImageV2($vImgName, $iMemberId,$_SESSION['sess_user']);
