...
$client->setApplicationName($GOOGLE_PLUS_APP_NAME);
$client->setClientId($GOOGLE_PLUS_OAUTH_CLIENT_ID);
$client->setClientSecret($GOOGLE_PLUS_OAUTH_CLIENT_SECRET);
$client->setRedirectUri($GOOGLE_PLUS_OAUTH_REDIRECT_URI);
$client->setScopes(array(Google\Service\Oauth2::USERINFO_EMAIL, Google\Service\Oauth2::USERINFO_PROFILE, Google\Service\Oauth2::OPENID));
...
if ($client->getAccessToken()) {
	...
	try {		
		...
		if($_SESSION['fb_user'] == 'rider') {
			...
			if($femail != '' || $fbid != '') {
				$sqll001 = " (IF('$femail'!='',vEmail = '$femail',0) OR IF('$fbid'!='',vFbId = '$fbid'  AND eSignUpType = 'Google',0)) AND eStatus != 'Deleted'"; //bc login with same user if rename email from admin side
