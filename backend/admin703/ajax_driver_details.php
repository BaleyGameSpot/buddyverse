...
if ($reg_date1 != "0000-00-00 00:00:00") {
    ...
    $date_format_data_array = array(
        'tdate' => (!empty($data_driver[0]['vTimeZone'])) ? converToTz($reg_date1,$data_driver[0]['vTimeZone'], $systemTimeZone) : $reg_date1,
        'langCode' => $default_lang,
        'DateFormatForWeb' => 1
    );
    $get_registration_date_format = DateformatCls::getNewDateFormat($date_format_data_array);
    $reg_date = $get_registration_date_format['tDisplayDate'];//DateTime($reg_date1);
} else {
    $reg_date = "";
}
// exit;
...
if (scount($usertotaltrips) > 0) {
    for ($i = 0; $i < scount($usertotaltrips); $i++) {
