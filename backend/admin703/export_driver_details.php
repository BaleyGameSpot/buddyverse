...
if($action != '')
{
  ...
  if($iCompanyId!=''){
      ...
      if(scount($db_driver2)>0)
  		{
  			for($i=0;$i<scount($db_driver2);$i++)
  			{
...
for($j=0;$j<scount($db_trip);$j++)
{
    ...
    $data .= ($db_trip[$j]['fCommision'] != "" && $db_trip[$j]['fCommision'] != 0) ? formateNumAsPerCurrency($db_trip[$j]['fCommision'],'')."\t" : "- \t";
    $data .= ($db_trip[$j]['fDiscount'] != "" && $db_trip[$j]['fDiscount'] != 0) ? formateNumAsPerCurrency($db_trip[$j]['fDiscount'],'')."\t" : "- \t";
    $data .= ($db_trip[$j]['fWalletDebit'] != "" && $db_trip[$j]['fWalletDebit'] != 0) ? formateNumAsPerCurrency($db_trip[$j]['fWalletDebit'],'')."\t" : "- \t";
