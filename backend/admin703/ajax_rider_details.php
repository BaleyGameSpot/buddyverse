...
if ($reg_date1 != "0000-00-00 00:00:00") {
    ...
    $date_format_data_array = array(
        'tdate' =>  (!empty($data_user[0]['vTimeZone'])) ? converToTz($reg_date1,$data_user[0]['vTimeZone'], $systemTimeZone) : $reg_date1,
        'langCode' => $default_lang,
        'DateFormatForWeb' => 1
    );
    $get_reg_format = DateformatCls::getNewDateFormat($date_format_data_array);
    $reg_date = $get_reg_format['tDisplayDate'];//DateTime($reg_date1);
}
else {
    $reg_date = "";
}
if ($data_user[0]['vImgName'] != "" && file_exists($tconfig["tsite_upload_images_passenger_path"] . '/' . $iUserId . '/2_' . $data_user[0]['vImgName'])) $image_path = $tconfig["tsite_upload_images_passenger"] . '/' . $iUserId . '/2_' . $data_user[0]['vImgName'];
else {
        ...
        <td>
            <table border="0" width="100%" height="150px" cellspacing="5" cellpadding="5px">
                <tr>
                    ...
                    <td><?= clearName($data_user[0]['Name']) ?></td>
                </tr>
                <tr>
