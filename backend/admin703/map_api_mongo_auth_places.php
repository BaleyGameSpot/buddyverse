...
$DbName = TSITE_DB;
$TableName = "auth_accounts_places";
if ($id != '' || $id != 0 || $eStatus != '' || $keyword != '' || $EntityType_option != '') {
    if ($id != '') {$searchQuery['vServiceId'] = intVal($id);}
    // $regex = new Regex($text, 's');
    if ($keyword != '') {$searchQuery['auth_key'] = $keyword;}
    if ($eStatus != '') {$searchQuery['eStatus'] = $eStatus;}
...
// echo "<pre>"; print_r($days_per_account_arr_new); exit;
?>
<!DOCTYPE html>
<html lang="en">
    <!-- BEGIN HEAD-->
    <head>
        <meta charset="UTF-8" />
        <title><?=$SITE_NAME?> |  API Keys (<?php echo $Servicetitle; ?>)</title>
        ...
        <div id="wrap">
            ...
            <div id="content">
                <div class="inner">
                    ...
                    <hr />
                    <div class="base-api-key"><?= !empty($GOOGLE_SEVER_GCM_API_KEY) ? $GOOGLE_SEVER_GCM_API_KEY : "Api Key Not Configured." ?></div>
                </div>
                                        ...
                                        <table class="table table-striped table-bordered table-hover">
                                            ...
                                            <tbody>
                                                    ...
                                                    if (!empty($data_drv)) {
                                                        for ($i = 0; $i < scount($data_drv); $i++) {
                                                            ...
                                                            if($data_drv[$i]['eStatus'] == "Active") {
                                                                $range = "<br>(Key to be used from " . $days_per_account_arr_new[$i]['days'][0] . ' - ' . $days_per_account_arr_new[$i]['days'][scount($days_per_account_arr_new[$i]['days']) - 1] . ")";
                                                            }

                                                            $api_key = $data_drv[$i]['auth_key'];
                                                            $api_key_status = $data_drv[$i]['eStatus'];
                                                ...
                                                <tr class="gradeA">
                                                    ...
                                                    <td style="word-break: break-all;" class="align-center">Key <?= $i+1 ?><?= $range ?></td>
                                                    <td style="word-break: break-all;" class="align-center">
                                                        <?= $api_key; ?>
                                                    </td>
                                                    <td align="center" class="action-btn001">
                                                        <button type="button" class="btn btn-info" onclick="addApiKey(this)" data-id="<?= $data_drv[$i]['_id']['$oid']; ?>" data-sid="<?= $data_drv[$i]['vServiceId']; ?>" data-authkey="<?= $api_key; ?>" data-apikeytitle="<?= $data_drv[$i]['vTitle']; ?>" data-order="<?= $data_drv[$i]['vUsageOrder']; ?>" data-status="<?= $api_key_status; ?>" data-defaultkey="<?= $data_drv[$i]['eDefault']; ?>">Add New / Update</button>
                                                    </td>
                                                </tr>
                                                <?php
                                                    }
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <form id="_authmongoplaces_form" name="_authmongoplaces_form" method="post" action="">
                                <input type="hidden" name="vServiceAccountId" id="vServiceAccountId" value="<?= $id; ?>"/>
