...
if($action != '' && $action == "export")
{
  ...
  for($j=0;$j<scount($db_trip);$j++)
  {
        ...
        $data .= ($db_trip[$j]['vTripPaymentMode']=='Cash')?formateNumAsPerCurrency($db_trip[$j]['iFare'],''):'0';
        $data .= "\t";
    	  $data .= ($db_trip[$j]['vTripPaymentMode']=='Card')?formateNumAsPerCurrency($db_trip[$j]['iFare'],''):'0';
        $data .= "\n";
  }
}
