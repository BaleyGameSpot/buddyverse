...
for ($t = 0; $t < scount($getVehicleTypeData); $t++) {
...
for ($i = 0; $i < scount($getSubCategoryData); $i++) {
...
$srNo = 1;
for ($c = 0; $c < scount($getCategoryData); $c++) {
    ...
    if (scount($catArr) > 0) {
        ...
        for ($s = 0; $s <= scount($categoryCount); $s++) {
            $displayOrder = ($s + 1);
